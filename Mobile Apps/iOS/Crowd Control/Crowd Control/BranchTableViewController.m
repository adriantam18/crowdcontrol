...
- (void)requestDataFromAPI {
    ...
    NSString *urlString = [NSString stringWithFormat:@"URL TO FETCH FROM",self.company];
    ...
    if([segue.identifier isEqualToString:@"toRooms"]){
        ...
        for(int i = 0; i < [self.branches count]; i++) {
            if (self.branches[i][@"address"] == selectedCell.textLabel.text) {
                ...
                roomController.branchId = sef.branches[i][@"branch_id"]
                ...
                NSString *openTime = self.branches[i][@"open_hours"];
                NSString *closeTime = self.branches[i][@"close_hours"];
